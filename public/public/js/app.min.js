...
//                        var thisShareNoteDefault = shareNotebookDefault[ToUserId];
                        ...
                        for(var i in shareNotebookDefault[ToUserId]) {
                            ...
                            if(curShareNoteData.NoteId == shareData.NoteId) {
                                showAlert("#shareMsg", "Sorry! This note has already been shared to the same user");
                                isSharedAlready = 1;
                                break;
                            }
                        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (res.status == "success") { // 如果成功
                    console.log("success");
                    if(Share.dialogIsNote) {
                        var receiverUserId = res.receiverUserId;
//                        var thisShareNoteDefault = shareNotebookDefault[ToUserId];
                        if( shareNotebookDefault[receiverUserId] == undefined) {
                            shareNotebookDefault[receiverUserId] = [];
                        }
                        var isSharedAlready = 0;
                        for(var i in shareNotebookDefault[receiverUserId]) {
                            var curShareNoteData = shareNotebookDefault[receiverUserId][i];
                            if(curShareNoteData.NoteId == shareData.NoteId) {
                                showAlert("#shareMsg", "Sorry! This note has already been shared to the same user");
                                isSharedAlready = 1;
                                break;
                        ...
                        if(!isSharedAlready) {
                            shareNotebookDefault[receiverUserId].push(shareData);
                        }
                    }
            
                    if(!hasShared) {
                        var text = '<button class="btn btn-success btn-share-or-not"> Shared </button>';
                        $ptr.data('uid', 'xxxxxx');
                    } else {
                    ...
                    showAlert("#shareMsg", "Sorry, share failed");
                }
            },
            error: function (xhr, status, error) {
                console.log('Error: ' + error.message);
